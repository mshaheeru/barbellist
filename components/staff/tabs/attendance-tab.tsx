...
export function StaffAttendanceTab({ staff }: StaffAttendanceTabProps) {
  const now = new Date();
  ...
  const checkInDays = new Set(
    staff.attendance_month.map((a) => new Date(a.check_in_at).getDate()),
  );

  const calendarDays = getMonthDays(year, month);
  const weekDays = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
  const recent = staff.attendance_month.slice(0, 20);

  return (
    <>
      <div className={styles.card}>
        <div className={styles.cardTitle}>
          {monthLabel} · {staff.attendance_month.length} check-ins · Streak{" "}
