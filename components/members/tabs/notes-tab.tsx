...
export function NotesTab({ memberId, initialNotes }: NotesTabProps) {
  ...
  const handleAdd = () => {
    ...
    startTransition(async () => {
      const { error: err } = await addMemberNote(memberId, text);
      if (err) {
        setError(err);
        return;
      }
  ...
  const handleSaveEdit = (noteId: string) => {
    ...
    setError(null);
    startTransition(async () => {
      const { error: err } = await editMemberNote(memberId, noteId, editText);
      if (err) {
        setError(err);
        return;
      }
  ...
  const handleDelete = (noteId: string) => {
    setError(null);
    startTransition(async () => {
      const { error: err } = await removeMemberNote(memberId, noteId);
      if (err) {
        setError(err);
        return;
      }
